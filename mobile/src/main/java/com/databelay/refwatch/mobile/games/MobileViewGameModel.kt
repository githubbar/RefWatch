package com.databelay.refwatch.mobile.games // Or your package
...
import com.databelay.refwatch.mobile.di.UserIdFlow
...
class MobileGameViewModel @Inject constructor(
    ...
    @UserIdFlow private val userIdFlow: Flow<String?> // Inject the Flow<String?>
    ...
    // Phone side (MobileGameViewModel or a test button)
    fun sendPing() {
        val path = "/simple_ping_test"
        Log.d("PhonePing", "Sending ping to $path")
        val putDataMapReq = PutDataMapRequest.create(path)
        putDataMapReq.dataMap.putString("ping_data", "Hello Watch at ${System.currentTimeMillis()}")
        putDataMapReq.dataMap.putLong("ts", System.currentTimeMillis()) // Force update
        putDataMapReq.setUrgent()
        viewModelScope.launch {
            try {
                Wearable.getDataClient(getApplication()).putDataItem(putDataMapReq.asPutDataRequest()).await()
                Log.i("PhonePing", "Ping sent successfully to $path")
            } catch (e: Exception) {
                Log.e("PhonePing", "Ping send FAILED", e)
            }
        }
    }
